...
function StatCardSkeleton() {
  return (
    <div className="bg-white p-4 rounded-xl border border-zinc-100 shadow-sm">
      ...
      <Skeleton width="55%" height={18} className="mb-2" />
    ...
    <div className="p-6 bg-light-gray min-h-screen">
        ...
        <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {isLoading
            ? Array.from({ length: 4 }).map((_, index) => (
                <StatCardSkeleton key={`self-assessment-skeleton-${index}`} />
              ))
      ...
      <div className="mb-10">
        ...
        <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {isLoading
            ? Array.from({ length: 4 }).map((_, index) => (
                <StatCardSkeleton key={`assured-certification-skeleton-${index}`} />
              ))
              ...
              {isLoading ? (
                Array.from({ length: 4 }).map((_, index) => (
                  <div
                    ...
                    className="flex items-center gap-4 p-4 rounded-md border border-zinc-100"
                    style={{ backgroundColor: "#f5f5f5" }}
                  >
                    <Skeleton width={48} height={48} borderRadius={8} />
                    <div className="flex-1">
        ...
        <div className="bg-white rounded-xl border" style={{ borderColor: "#E6E6E6" }}>
          ...
          <div className="p-3 md:p-5">
            <div className="flex flex-col gap-4">
              {isLoading ? (
                <div
                  className="flex items-center gap-4 p-4 rounded-md relative overflow-hidden"
                  style={{ backgroundColor: "#f5f5f5" }}
                >
                  <div className="absolute left-0 top-0 bottom-0 w-[3px] rounded-l-md" style={{ backgroundColor: "#262626" }}></div>
