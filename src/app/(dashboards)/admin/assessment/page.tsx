...
type AssessmentFilters = {
  organizationId: string;
  status: 'all' | 'in_progress' | 'submitted' | 'ai_reviewing' | 'completed' | 'expired';
    ...
    <div className="bg-white rounded-xl p-3 md:p-6 shadow-sm">
      <div className="flex items-start gap-2 md:gap-4">
        ...
        <div className="flex-1">
          ...
          <p 
            className="text-secondary font-medium md:text-[26px] text-[20px]"
          >
            {count}
          </p>
        </div>
...
export default function AssessmentPage() {
  ...
  const [metrics, setMetrics] = useState<AssessmentMetricsApiResponse['data'] | null>(null);
  ...
  const buildFilterParams = useCallback(() => {
    ...
    if (filters.assessmentType !== 'all') params.assessmentType = filters.assessmentType;
  ...
  const fetchAssessmentMetrics = useCallback(async () => {
    ...
    try {
      const response = await axiosInstance.get<AssessmentMetricsApiResponse>('/admin/assessments/metrics');
  ...
  const fetchAssessments = useCallback(async () => {
    ...
    setError('');
    try {
      const response = await axiosInstance.get<AssessmentsApiResponse>('/admin/assessments', {
        params: {
    ...
    () => [
        ...
        cell: ({ row, getValue }) => {
          ...
          return (
            <span
              ...
              style={
                badgeColor
                  ? { backgroundColor: `${badgeColor}14`, color: badgeColor, borderColor: `${badgeColor}55` }
                  : undefined
              }
            >
              {badge}
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <div className="flex items-center justify-center">
            <button 
              onClick={() => router.push(`/admin/assessment/details?id=${row.original.assessmentId}`)}
            ...
            <div className="flex flex-col lg:flex-row items-stretch lg:items-center gap-2 md:gap-3 flex-1">
              ...
              <Dropdown
                ...
                onChange={(e) =>
                  setFilters((prev) => ({
                    ...prev,
                    assessmentType: e.target.value as AssessmentFilters['assessmentType'],
                  }))
                }
                placeholder=""
                options={[
                  ...
                  {headerGroup.headers.map((header) => (
                    <th
                      ...
                      className={`px-2 md:px-4 py-2 md:py-4 ${
                        header.id === 'action' || header.id === 'blockStatus' ? 'text-center' : 'text-left'
                      }`}
                      style={{
                  ...
                  >
                    {row.getVisibleCells().map((cell) => (
                        <td
                          key={cell.id}
                          className={`px-2 md:px-4 py-2 md:py-4 ${
                          cell.column.id === 'action' || cell.column.id === 'blockStatus' ? 'text-center' : ''
                        }`}
                        style={{
