...
function AssessmentDetailsContent() {
  ...
  const loaderFinishTimeoutRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  const [assessmentQuestions, setAssessmentQuestions] = useState<AssessmentQuestion[]>([]);
  const [assessmentRefreshKey, setAssessmentRefreshKey] = useState(0);
  const [aiFlaggedAnswerIds, setAiFlaggedAnswerIds] = useState<Set<string>>(new Set());
  const [aiFlagReasonsById, setAiFlagReasonsById] = useState<Record<string, string>>({});
    ...
    const fetchAssessmentDetails = async () => {
      ...
      try {
        ...
        if (axios.isAxiosError(err)) {
          console.error('assessment details error response:', err.response?.data);
        }
  ...
  useEffect(() => {
    ...
    const fetchAssessmentQuestions = async () => {
      try {
        const response = await axiosInstance.get<AssessmentQuestionsApiResponse>(
    ...
    const fetchAiReviewFlags = async () => {
      try {
        ...
        if (!isCancelled) {
          ...
          if (axios.isAxiosError(err)) {
            console.error('ai review flags error response:', err.response?.data);
          }
          setAiFlaggedAnswerIds(new Set());
  ...
  const getDisplayName = (assignee: NamedAssignee | string | null | undefined): string => {
    if (!assignee) return '';
  ...
  const aiStatusLabel = isAiFailed ? 'AI Failed' : isAiFlagged ? 'AI Flagged' : 'AI Passed';
  ...
  const submittedLabel =
    formatDate(assessmentDetails?.submittedAt) || fallbackAssessmentData.submitted;
  const hasAssignedAuditor = !!getDisplayName(assessmentDetails?.assignedAuditor);
  const hasAssignedReviewer = !!getDisplayName(assessmentDetails?.assignedReviewer);
  ...
  const assessmentData = {
    organisation: assessmentDetails?.organizationName ?? fallbackAssessmentData.organisation,
    certification: assessmentDetails?.certificateName ?? fallbackAssessmentData.certification,
  ...
  const tags: Array<{ label: string; className: string; style?: React.CSSProperties }> = [];
  ...
  if (isSelfAssessment) {
    ...
    tags.push({
      ...
      className: 'px-3 py-1 border border-black rounded-lg text-xs font-medium text-secondary',
  ...
  if (assessmentData.aiFlagged) {
    tags.push({
      ...
      style: { backgroundColor: '#ffe6e6', color: '#DC2626', borderColor: '#DC2626' },
    });
  } else {
    tags.push({
      label: 'AI Passed',
  ...
  if (hasAssignedAuditor || hasAuditorReviewResponse) {
    ...
    if (hasAuditorReviewResponse) {
      tags.push({
        ...
        className: 'px-3 py-1 border rounded-lg text-xs font-medium',
        style: { backgroundColor: '#f0fdf4', color: '#00B448', borderColor: '#00B448' },
      });
      tags.push({
        label: isAuditFailed ? 'Failed' : 'Passed',
        className: `px-3 py-1 border rounded-lg text-xs ${isAuditFailed ? 'font-semibold' : 'font-medium'}`,
        style: isAuditFailed
          ? { backgroundColor: '#fef2f2', color: '#DC2626', borderColor: '#DC2626' }
          : { backgroundColor: '#f0fdf4', color: '#00B448', borderColor: '#00B448' },
      });
    } else if (hasAssignedAuditor) {
      tags.push({
        label: 'Audit in Process',
        className: 'px-3 py-1 border rounded-lg text-xs font-medium',
        style: { backgroundColor: '#f6f0e2', color: '#FAAB00', borderColor: '#FAAB00' },
      });
    }
  }
  ...
  const [isAssignAuditorModalOpen, setIsAssignAuditorModalOpen] = useState(false);
  ...
  const [assignAuditorError, setAssignAuditorError] = useState<string | null>(null);
  ...
  const [assignReviewerError, setAssignReviewerError] = useState<string | null>(null);
  const [isBlockCertificationModalOpen, setIsBlockCertificationModalOpen] = useState(false);
  ...
  const [blockCertificationError, setBlockCertificationError] = useState<string | null>(null);
  ...
  const questionsData = useMemo(() => {
    ...
    assessmentQuestions.forEach((item, index) => {
      ...
      const subsectionTitle =
        item.sub_section_name?.trim() || item.section_name?.trim() || 'Sub Section';
      ...
      const attachmentLabel = attachmentUrl ? getAttachmentLabel(attachmentUrl) : null;
    ...
    const fetchAuditors = async () => {
      ...
      try {
        ...
        const mappedData: AuditorOption[] = apiData.map((auditor: any) => {
          const locationParts = [auditor.city, auditor.state, auditor.country].filter(Boolean);
          const statusRaw =
          ...
          return {
            ...
            status: statusRaw === 'available' || statusRaw === 'active' ? 'available' : 'busy',
          };
        });
        ...
        if (!isCancelled) {
          ...
          if (axios.isAxiosError(err)) {
            setAuditorsError(err.response?.data?.message || 'Failed to load auditors');
          } else {
    ...
    const fetchReviewers = async () => {
      ...
      try {
        ...
        if (!isCancelled) {
          ...
          if (axios.isAxiosError(err)) {
            setReviewersError(err.response?.data?.message || 'Failed to load reviewers');
          } else {
  ...
  const handleAssignAuditor = async () => {
    ...
    try {
      ...
      const response = await axiosInstance.post('/auditors/assign-assessment', payload);
  ...
  const handleAssignReviewer = async () => {
    ...
    try {
      ...
      const response = await axiosInstance.post('/reviewers/assign-assessment', payload);
  ...
  return (
    <div className="bg-light-gray p-3 md:p-6 min-h-screen">
      ...
      <div className="flex flex-wrap gap-3 mb-6">
        {tags.map((tag, index) => (
          <div key={`${tag.label}-${index}`} className={tag.className} style={tag.style}>
            {tag.label}
          </div>
        ))}
      </div>
      ...
      {isSelfAssessment && (
        <div className="bg-white rounded-xl p-4 md:p-6 shadow-sm mb-6">
          <h3 className="text-lg font-semibold text-secondary mb-1">AI Reason</h3>
      ...
      {hasAuditorReviewResponse && (
        <div className="mt-6">
          <div className="bg-white rounded-xl p-4 md:p-6 shadow-sm mb-6">
            <h3 className="text-lg font-semibold text-secondary mb-1">Reviews from auditor</h3>
            <p className="text-xs md:text-sm font-normal text-gray leading-relaxed">
                      ...
                      {section.subsections.map((subsection) => (
                        ...
                        <div className="mt-2 space-y-4">
                          {subsection.questions.map((question, questionIndex) => (
                            <div
                              key={question.id}
                                  className={`bg-white rounded-lg p-4 md:p-5 border ${question.aiFlagged
                                      ? 'border-red-300'
                                      : 'border-zinc-200'
                                    }`}
                            >
                                  ...
                                  <div className="ml-8 mb-1">
                                    <p className="text-xs md:text-sm font-normal" style={{ color: "#999999" }}>
                                  {question.answer}
                                </p>
                              </div>
                                ...
                                >
                                  ...
                                  <span>{question.attachmentLabel}</span>
                                  <svg
                                    width="20"
                                    height="20"
                                    viewBox="0 0 20 20"
                                  ...
                                  >
                                    ...
                                    />
                                    ...
                                    ) : question.aiFlagged && question.isFileQuestion ? (
                                      <div
                                      ...
                                      >
                                        ...
                                        <span>No supporting documentation provided</span>
                                      </div>
                                    ) : null}
                              </div>

                                  {question.aiFlagged && question.aiFlagReason ? (
                                    <div
          ...
          <div className="flex flex-wrap gap-3 md:gap-4">
            ...
            <Button
              ...
              className={`px-4 py-2 md:px-6 md:py-3 border rounded-xl ${
                isCertificateBlocked ? 'cursor-not-allowed opacity-70' : 'hover:bg-red-100'
              }`}
              style={{
                backgroundColor: '#ffe6e6',
                color: '#DC2626',
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-xl shadow-lg w-full max-w-xl mx-4 p-4 md:p-6">
            ...
            <div className="flex justify-end gap-3 mt-8">
              ...
              >
                {isBlockingCertification ? 'Blocking...' : 'Block Certification'}
              </button>
            </div>
          </div>
        </div>
          ...
          <div className="relative bg-white rounded-xl shadow-lg w-full max-w-xl mx-4 p-4 md:p-6">
            ...
            <div className="flex items-start justify-between mb-4">
              <div>
                ...
                <p className="text-xs md:text-sm" style={{ color: '#999999' }}>
                  Select an auditor to review the assessment for {assessmentData.organisation}
                </p>
              </div>
              <button
                onClick={() => {
            ...
            <div className="space-y-6 mt-6">
              ...
              <div style={{ background: '#F6F6F6' }} className="px-4 py-3 rounded-lg">
              ...
              <div>
                <label htmlFor="audit-date" className="block text-sm font-normal text-gray-500 mb-2">
                  Audit Date (Optional)
                </label>
                <input
                  id="audit-date"
              ...
              <div>
                ...
                >
                  ...
                  {auditorOptions.slice(0, auditorVisibleCount).map((auditor) => {
                    const isSelected = selectedAuditor === auditor.id;
                    const hasProfilePicture = auditor.profile_picture_url && auditor.profile_picture_url.trim() !== '';
                        ...
                        <div className="flex items-start gap-4">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-2 mb-2">
                              ...
                              >
                                {auditor.status === 'available' ? 'Available' : 'Busy'}
                              </span>
                            </div>
          ...
          <div className="relative bg-white rounded-xl shadow-lg w-full max-w-xl mx-4 p-4 md:p-6">
            ...
            <div className="flex items-start justify-between mb-4">
              <div>
                ...
                <p className="text-xs md:text-sm" style={{ color: '#999999' }}>
                  Select a reviewer to review the assessment for {assessmentData.organisation}
                </p>
              </div>
              <button
                onClick={() => {
            ...
            <div className="space-y-6 mt-6">
              ...
              <div>
                ...
                >
                  ...
                  {reviewerOptions.slice(0, reviewerVisibleCount).map((reviewer) => {
                    const isSelected = selectedReviewer === reviewer.id;
                    const hasProfilePicture = reviewer.profile_picture_url && reviewer.profile_picture_url.trim() !== '';
